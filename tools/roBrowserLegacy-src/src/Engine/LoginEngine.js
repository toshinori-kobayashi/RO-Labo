...
class LoginEngine {
	...
	static init(server) {
		...
		// Setup network (packet) charset based on LangType.
		// Only the network side is changed here; the GRF/DB charset (TextEncoding.userCharset) is untouched.
		// An explicit server.networkCharset (ex: 'windows-932') overrides the langtype detection.
		const charset =
			Configs.get('networkCharset') || TextEncoding.detectEncodingByLangtype(Session.LangType, Configs.get('disableKorean'));
		TextEncoding.setNetworkCharset(charset);

		console.log('%c[LOGIN] Language Type: ', 'color:#007000', Session.LangType);
		console.log('%c[LOGIN] Network Encoding: ', 'color:#007000', TextEncoding.networkCharset);
		console.log('%c[LOGIN] GRF Encoding: ', 'color:#007000', TextEncoding.userCharset);
