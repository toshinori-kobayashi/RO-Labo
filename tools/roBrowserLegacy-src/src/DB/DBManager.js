...
class DB {
	...
	static async lazyInit() {
		...
		if (Configs.get('loadLua')) {
			...
			// jRO skill display names and descriptions. Does not replace SP / level / requirements.
			loadSkillDisplayNames(onLoad());
			loadSkillDescriptions(onLoad());
...
/**
 * Overlay Japanese skill display names from the local client (jRO `skillnamelist.lub`).
 * That file is Gravity LuaP bytecode, which the Lua 5.1 runtime cannot execute.
 * String constants are read directly and matched to SkillConst (NV_BASIC and the rest).
 * Only SkillInfo[].SkillName is replaced. MaxLv / SpAmount / ranges stay on the built-in table.
 *
 * @param {function} onEnd
 */
function loadSkillDisplayNames(onEnd) {
	const namePath = 'data/lua files/skillinfoz/skillnamelist.lub';

	Client.loadFile(
		namePath,
		function (file) {
			try {
				const buf = file instanceof ArrayBuffer ? new Uint8Array(file) : file;
				const applied = applySkillDisplayNames(buf);
				console.log('[skill-jp] applied display names:', applied);
			} catch (error) {
				console.error('[skill-jp] failed to read skillnamelist.lub', error);
			}
			onEnd();
		},
		function () {
			console.warn('[skill-jp] skillnamelist.lub was not found');
			onEnd();
		}
	);
}
...
/**
 * skillnamelist.lub stores SKID tokens and CP932 display names as length-prefixed constants.
 * The name sits a few constants after its SKID token.
 *
 * @param {Uint8Array} buf
 * @return {number} names written onto SkillInfo
 */
function applySkillDisplayNames(buf) {
	const strings = [];
	for (let i = 0; i + 5 < buf.length; i++) {
		const size = buf[i] | (buf[i + 1] << 8) | (buf[i + 2] << 16) | (buf[i + 3] << 24);
		if (size < 2 || size > 120 || i + 4 + size > buf.length || buf[i + 4 + size - 1] !== 0) {
			continue;
		}
		const raw = buf.subarray(i + 4, i + 4 + size - 1);
		let binary = false;
		...
		const text = userStringDecoder.decode(raw, userCharpage);
		if (!text) {
			continue;
		}
		const isSkillId = Object.prototype.hasOwnProperty.call(SKID, text);
		const isLabel = text === 'SKID' || text === 'SkillName' || text === 'SKILL_INFO_LIST';
		const hasJapanese = /[\u3040-\u30ff\u4e00-\u9fff]/.test(text);
		if (!isSkillId && !isLabel && !hasJapanese) {
			continue;
		}
		strings.push(text);
		i += 3 + size;
	...
	const skip = new Set(['SKID', 'SkillName', 'SKILL_INFO_LIST']);
	let applied = 0;
	for (let i = 0; i < strings.length; i++) {
		const token = strings[i];
		if (!Object.prototype.hasOwnProperty.call(SKID, token)) {
			continue;
		}
		const id = SKID[token];
		if (!SkillInfo[id]) {
		...
		let name = '';
		for (let j = i + 1; j < strings.length && j < i + 6; j++) {
			const next = strings[j];
			if (skip.has(next)) {
				continue;
			}
			if (Object.prototype.hasOwnProperty.call(SKID, next)) {
				break;
			}
			name = next;
			break;
...
/**
 * Overlay Japanese skill descriptions from jRO `skilldescript.lub`.
 * Same LuaP bytecode as the name list, so constants are read directly.
 * Each SKID is followed by description lines. A leading number on a color line
 * (for example `14^777777`) is a client-internal line id and is not shown.
 *
 * @param {function} onEnd
 */
function loadSkillDescriptions(onEnd) {
	Client.loadFile(
		'data/lua files/skillinfoz/skilldescript.lub',
		function (file) {
			try {
				const buf = file instanceof ArrayBuffer ? new Uint8Array(file) : file;
				const applied = applySkillDescriptions(buf);
				console.log('[skill-jp] applied descriptions:', applied);
			} catch (error) {
				console.error('[skill-jp] failed to read skilldescript.lub', error);
			}
			onEnd();
		},
		function () {
			console.warn('[skill-jp] skilldescript.lub was not found');
			onEnd();
		}
	);
}
 ...
 * @return {number}
 */
function applySkillDescriptions(buf) {
	const strings = [];
	for (let i = 0; i + 6 < buf.length; i++) {
		if (buf[i] !== 4) {
			continue;
		}
		const size = buf[i + 1] | (buf[i + 2] << 8) | (buf[i + 3] << 16) | (buf[i + 4] << 24);
		if (size < 2 || size > 2000 || i + 5 + size > buf.length || buf[i + 4 + size] !== 0) {
			continue;
		}
		const raw = buf.subarray(i + 5, i + 4 + size);
		let binary = false;
		for (let b = 0; b < raw.length; b++) {
			const byte = raw[b];
			if (byte !== 9 && byte !== 10 && byte !== 13 && byte < 0x20) {
				binary = true;
				break;
			}
		}
	...
	const skip = new Set(['SKILL_DESCRIPT', 'SKID']);
	...
	for (const text of strings) {
		...
		if (Object.prototype.hasOwnProperty.call(SKID, text)) {
			current = text;
			if (!grouped.has(current)) {
				grouped.set(current, []);
			}
			continue;
		}
		...
		grouped.get(current).push(text.replace(/^\d+(?=\^)/, ''));
	}

	let applied = 0;
	for (const [token, lines] of grouped) {
		const id = SKID[token];
		if (!lines.length || typeof id !== 'number') {
			continue;
		}
		SkillDescription[id] = lines.join('\n');
		applied++;
	}
	return applied;
}
