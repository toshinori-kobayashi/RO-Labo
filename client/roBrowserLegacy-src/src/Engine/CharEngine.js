...
function onDeleteRequest(charID) {
	...
	// msgstringtable #19 still says "registered e-mail". PACKETVER 20211103
	// confirms with the 8-digit code (YYYYMMDD); the packet field stays 6 bytes.
	const deleteConfirmText =
		'キャラクターを削除しますか？\n削除するには、登録時に案内された削除確認コードを入力してください。\n8桁の数字で入力してください。';
	_ui_box = UIManager.showPromptBox(deleteConfirmText, 'ok', 'cancel', onOk, onCancel);
	const promptText = _ui_box._shadow && _ui_box._shadow.querySelector('.text');
	if (promptText) {
	...
	// PACKETVER>20100803 keeps the existing packet path: the user types 8 digits
	// (YYYYMMDD) and deleteCharacter() sends substring(2) as the 6-byte YYMMDD field.
	function onSubmit(input) {
		_inputValue = input;
		InputBox.remove();
		_ui_box.remove();

		if (PACKETVER.value > 20100803 && !/^\d{8}$/.test(String(_inputValue))) {
			_overlay.remove();
			UIManager.showMessageBox('削除確認コードは8桁の数字で入力してください。', 'ok');
			onDeleteAnswer({ Result: -2 });
			return;
		}
