...
DataView.prototype.setString = function SetString(offset, str, len) {
	let i, count;
	// network charset (langtype based); for fixed-length fields truncate on a character boundary
	const data = TextEncoding.encodeNetwork(String(str), len || 0);

	for (i = 0, count = len || data.length; i < count; ++i) {
		this.setUint8(offset + i, i < data.length ? data[i] : 0);
	}
};
...
BinaryWriter.prototype.setString = BinaryWriter.prototype.writeString = function setString(str, length) {
	...
	// network charset (langtype based); for fixed-length fields truncate on a character boundary
	const data = TextEncoding.encodeNetwork(str, length || 0);
	let i;
	const count = length || data.length;

	// Variable-length field: multi-byte charsets make the byte length larger than str.length,
	// so the buffer (allocated with str.length) has to grow and the packet size be fixed.
	...
	for (i = 0; i < count; ++i) {
		this.view.setUint8(this.offset + i, i < data.length ? data[i] : 0);
	}
