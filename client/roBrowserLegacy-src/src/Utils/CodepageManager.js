...
/**
 * Unicode variants that Windows IME / other platforms may produce but that do not exist in the
 * CP932 (Microsoft Shift_JIS) table. Normalize them to their CP932 counterparts before encoding
 * so they are not turned into '?'.
 */
const CP932_ENCODE_NORMALIZE = {
...
const CP932_NORMALIZE_RE = /[\u301c\u2016\u2212\u00a2\u00a3\u00ac]/g;
const CP932_NAMES = ['shift-jis', 'shiftjis', 'shift_jis', 'sjis', 'windows-932', 'windows932', 'cp932', 'ms932'];
...
const CodepageManager = {
	/**
	 * Charset used for strings exchanged with the game server (network packets).
	 * Kept separate from userCharset (GRF / client files) so that changing one never affects the other.
	 * Set from the server langtype in LoginEngine (or explicitly with server.networkCharset).
	 */
	networkCharset: 'windows-1252',

	/*
	 * @param {string} charset - Charset to use for network packet strings.
	 */
	setNetworkCharset: function setNetworkCharset(charset) {
		if (!iconv.encodingExists(charset)) {
			console.error(`[TextEncoding.setNetworkCharset] Invalid charset: "${charset}".`);
			return;
		}
		this.networkCharset = charset;
	},

	/*
	 * Decode a string received from the server.
	 * UTF-8 is tried first (some servers are UTF-8), then the network charset.
	 * @param {Uint8Array} data - Raw byte data.
	 * @returns {string}
	 */
	decodeNetwork: function decodeNetwork(data) {
		if (!(data instanceof Uint8Array)) {
			console.error(
				`[TextEncoding.decodeNetwork] Invalid input type: expected "Uint8Array", got "${typeof data}".`,
				data
			);
			return '';
		}
		return smartDecode(data, this.networkCharset);
	},

	/*
	 * Encode a string to send to the server, using the network charset.
	 * @param {string} str
	 * @param {number} maxBytes? - if set, the result is truncated on a character boundary to fit.
	 * @returns {Uint8Array}
	 */
	encodeNetwork: function encodeNetwork(str, maxBytes = 0) {
		if (typeof str !== 'string') {
			console.error(`[TextEncoding.encodeNetwork] Invalid input type: expected "string", got "${typeof str}".`, str);
			return new Uint8Array(0);
		}
		if (CP932_NAMES.indexOf(String(this.networkCharset).toLowerCase()) !== -1) {
			str = str.replace(CP932_NORMALIZE_RE, c => CP932_ENCODE_NORMALIZE[c]);
		}
		let data = iconv.encode(str, this.networkCharset);
		if (maxBytes > 0 && data.length > maxBytes) {
			// drop whole characters (not bytes) until it fits, so no dangling lead byte is sent
			const chars = Array.from(str);
			while (chars.length && data.length > maxBytes) {
				chars.pop();
				data = iconv.encode(chars.join(''), this.networkCharset);
			}
		}
		return data;
	},
