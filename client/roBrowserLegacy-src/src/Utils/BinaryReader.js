...
BinaryReader.prototype.getString = BinaryReader.prototype.readString = function getString(len) {
	...
	this.offset += len;
	// Network packet strings: UTF-8 first, then the server (langtype) charset.
	// GRF/file loaders use readBinaryString(), so the GRF charset is not involved here.
	return TextEncoding.decodeNetwork(bytes.subarray(0, realLen));
};
