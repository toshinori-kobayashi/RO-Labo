...
**ro-glue はパケットの中身も文字コードも意味的に変換しない、バイト列透過の中継です。** WS→TCP のリレーはサーバから届いたバイト列をそのまま `ws.send(d)` し、クライアントから届いたバイト列をそのまま `tcp.write(b)` するだけです（`client/ro-glue/server.mjs`）。GRF / loose の資産配信もファイルの中身を書き換えません。

| 責務 | 担当 | 根拠（実ファイル） |
|---|---|---|
| 描画、UI、入力、IME | Client | `client/roBrowserLegacy-src/src/UI/`, `src/Renderer/` |
...
| 静的配信、GRF / loose 資産配信 | Bridge | `client/ro-glue/server.mjs`（`/client/<path>`） |
| WS → TCP 中継（許可リストのみ、バイト列透過） | Bridge | `client/ro-glue/server.mjs`（`server.on('upgrade', ...)`、`allowSet` で 403 判定） |
...
| NPC 台詞・頭上名・独自 NPC | Server | `server/app/rathena/overlay-utf8/npc/` |
| Mob 表示名、MOTD、サーバメッセージ | Server | `mob_db.yml`、`motd`、`map_msg` |
| インフラ、デプロイ、バックアップ | Server | Terraform / SSM / DLM（`server/terraform/`） |

## 4. 通信境界

- HTTP は資産配信のみです: `GET /client/<path>`（loose → GRF の順で解決、なければ 404）、`POST /client/`（filter 検索）、`POST /client/batch`（一括取得）、`GET /__status`（状態確認）。
- WebSocket は `ws://127.0.0.1:<port>/<接続先 ip>:<接続先 port>` の形式で、1 接続 = TCP 1 本です。URL のホスト:ポートが glue の許可リスト（`--allow`）に無ければ `403 Forbidden` で切断されます（`client/ro-glue/server.mjs` の `server.on('upgrade', ...)`）。
...
| glue（資産配信・WS 中継） | 無変換（バイト列透過） | `client/ro-glue/server.mjs` |
...
| サーバ NPC 翻訳 | 本プロジェクト | リポジトリ管理（`server/app/rathena/overlay-utf8/npc/custom/jp/`） |
...
- サーバ: MariaDB がアカウント・キャラクター等を永続化します。EBS 上に配置し、日次 mysqldump と DLM スナップショットでバックアップします（詳細は [../server/docs/OPERATIONS.md](../server/docs/OPERATIONS.md)）。
...
- サーバ: `terraform apply` → アプリ一式を S3 へアップロード → SSM Association 経由で EC2 に配布 → EC2 上で `docker compose build` して再起動します（[../server/docs/OPERATIONS.md](../server/docs/OPERATIONS.md)）。
